...
class WAD_Reader:
    ...
    def read_texture_map(self, offset):
        ...
        tex_map.column_dir = read_4_bytes(
            offset + 16, byte_format="I")  # unused
    ...
    def read_patch_map(self, offset):
        ...
        patch_map.step_dir = read_2_bytes(
            offset + 6, byte_format="H")  # unused
        patch_map.color_map = read_2_bytes(
            offset + 8, byte_format="H")  # unused
        return patch_map
    ...
    def read_patch_column(self, offset):
        ...
        if patch_column.top_delta != 0xFF:
            ...
            patch_column.padding_post = read_1_byte(
                offset + 3 + patch_column.length)  # unused
    ...
    def read_palette(self, offset):
        # 3 bytes = B + B + B
        read_1_bytes = self.read_1_byte

        palette = []
        for i in range(256):
            r = read_1_bytes(offset + i * 3 + 0)
            g = read_1_bytes(offset + i * 3 + 1)
            b = read_1_bytes(offset + i * 3 + 2)
