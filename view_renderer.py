...
class ViewRenderer:
    ...
    def get_color(self, tex, light_level):
        ...
        if tex + str_light not in self.colors:
            ...
            color = color[0] * light_level, color[1] * \
                light_level, color[2] * light_level
    ...
    def draw_flat(self, tex_id, light_level, x, y1, y2, world_z):
        if y1 < y2:
            if tex_id == self.sky_id:
                tex_column = 2.2 * (self.player.angle +
                                    self.engine.seg_handler.x_to_angle[x])
    ...
    def draw_flat_col(screen, flat_tex, x, y1, y2, light_level, world_z,
        ...
        for iy in range(y1, y2 + 1):
            ...
            col = flat_tex[tx, ty]
            col = col[0] * light_level, col[1] * \
                light_level, col[2] * light_level
            screen[x, iy] = col
